* Sample Stata code -- adapted from Dylan Brewer
...
* Set up your working directories
	...
	* local datapath = "C:\Users\brewe\Dropbox (Personal)\teaching\Courses\BrewerPhDEnv\Homeworks\phdee-2024-db\homework1" // Typically, where you keep the data and where you want the outputs to go will be different.  In this sample code, this is not the case so I don't specify a data path.
	local outputpath = "C:\Users\Owner\Dropbox\Personal\Enviro Econ II\phdee-2023-DW\homework1\output" 
...
* Generate some random data
	...
	set obs 100 
	*100 observations, so our random number generator stops at 100
	
	gen x1 = rnormal(10,10)
	gen x2 = rnormal(20,15)
	gen x3 = rnormal(15,15) 
	*this variable we do not observe -- it is the error term
...
* Fit linear regression model
	...
	reg yvar x1 x2 
	*this is the basic regression.  It calculates standard errors assuming homoskedasticity by default.
	...
	* If we need to bootstrap ourselves (to incorporate two steps in the estimation for example), we can draw bootstrap samples and repeatedly estimate our regression:
		...
		mat betas = J(1000,3,.) 
		*pre-allocate a matrix for the outcomes of our 1000 regressions
	
		forvalues i = 1/1000 {
			preserve 
			*preserves the data as it was in the memory at this point
				bsample 
				*samples with replacement up to the number of observations
			...
			restore 
			*restores the data as you preserved it originally
		}
	...
	* You can write a program to get Stata to replace the covariance matrix with the bootstrapped covariance matrix.  Doing this will let you use postestimation commands like outreg2 that make creating tables really easy.
		...
		program define bootstrapsample, eclass
			...
			*rerun the regression
			ereturn repost V = A 
			*post the new covariance matrix as the covariance matrix V that Stata uses
		end
		
		bootstrapsample 
		*runs the program we wrote
